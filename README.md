...
Usage:         		'Save index data file and option data file in working 
	     	 	 directory and adjust the names as well as the parameters 
			 for the simulation and calculation (under (0)).' 
...
# information given by rugarch package for further informations
garchmodel		= "fGARCH"
submodel		= "GARCH"
...
dev.new()
par(mar=c(5, 5, 5, 5))
title		= paste("EPKs on",date, "for index price"
			,indexprice)
plot(temp[,1],EPK[,1],col=1,type="l", xlab=~S[T], ylab="EPK", cex=1.5,
	xlim=c(min(temp[,1]),max(temp[,1])),ylim=c(0,4), lwd=3, cex.lab=1,
	main=title)	
